...
app.get("/api/films", async (req, res) => {
    ...
    try {
        ...
        if (films.length != 0) {
            ...
            res.status(400).send("La requête est invalide")
        )
    } catch (error) {
...
app.post(
    ...
    async (req, res) => {
        ...
        if (utilisateurs.length > 0) {
            return res.status(400).json({ message: "Email already taken. Please use a different one." });
        }
        ...
        const nouvelUtilisateurId = await db.collection("utilisateurs").add(nouvelUtilisateur);
        
        // On valide/nettoie la donnée
        delete nouvelUtilisateur.mdp;

        // On renvoie true;
        res.status(200).json({message: `Great news, ${nouvelUtilisateur.courriel}! You're now a registered user.`});
    }
);
...
app.post("/utilisateurs/connexion",[], async (req, res) => {
    ...
    // Si non, erreur
    if (utilisateurs.length == 0) {
        return res.status(400).json({ message: "Email does not exist. Please try again." });
    }
    ...
    if (!estValide) {
        return res.status(400).json({ message: "Oops! The password you entered is incorrect." });
    }

    // On retourne les infos de l'utilisateur sans le mot de passe
    delete utilisateurAValider.mdp;
    res.status(200).json({ message: `Congratulations ${utilisateurAValider.courriel}! You have successfully logged in.` })
});
